...
export default function Home() {
  ...
  const handleUserClick = (u) => {
    ...
    setInput((cur) => {
      const base = cur.trim();
      if (base.startsWith(`@${u.username}`)) return cur;
      return `@${u.username} ${base}`.trim();
    });
    setPrivateTarget(u.username);
  };
    ...
    return (
      <main className="h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="w-full max-w-md bg-white rounded-xl shadow p-6">
          <h2 className="text-2xl font-semibold mb-4 text-center">Join Chat</h2>
          <input
          ...
          <p className="text-sm text-gray-500 mt-3">
            Server may adjust username to ensure uniqueness; you will be
            informed when registered.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col">
        <div className="sm:hidden bg-white border-b p-2 flex gap-2 overflow-x-auto">
          {users.map((u) => {
            const isMe = u.username === myName;
            return (
              <button
                ...
                className={`flex items-center gap-2 p-2 rounded ${
                  privateTarget === u.username ? 'bg-green-50' : 'bg-white'
                }`}
              >
                <span
        ...
        <div className="bg-white p-4 border-b flex items-center justify-between">
          ...
          <div>
            {privateTarget ? (
              ...
              <span className="text-sm text-gray-500">Public chat</span>
            )}
          </div>
        </div>
